 ...
 *  
 *  and multiply damping factors A and B with shear modulus
 *
 *  D. Koehn
 *  Kiel, 26.05.2017
 *  -----------------------------------------------------------------------------*/
...
void pml_pro_SH(struct PML_AC *PML_AC, struct waveAC *waveAC, struct matSH *matSH){
	...
	for (j=1;j<=NY;j++){
		...
		mu = (*matSH).vs[j][i] * (*matSH).vs[j][i] * (*matSH).rho[j][i];

                /* calculate real and imaginary parts of factors A, B and C and multiply A and B with shear modulus */
		(*PML_AC).Ar[j][i] = creal(mu*sy/sx);
		(*PML_AC).Ai[j][i] = cimag(sy/sx);

		(*PML_AC).Br[j][i] = creal(mu*sx/sy);
