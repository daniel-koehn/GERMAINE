...
fscanf(fp,"%s%s%s%s%s%s%s%s",str,str,str,str,str,str,str,str);
for (i=1;i<=nstage;i++){
      fscanf(fp,"%f%f%f%i%i%f%f%f",&PRO,&FC_low,&FC_high,&NF,&SPATFILTER,&FILT_SIZE_GRAD,&FILT_SIZE_GRAD1,&VREF);
}
